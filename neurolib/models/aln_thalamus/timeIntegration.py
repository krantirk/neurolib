...
def timeIntegration(params):
    ...
    rates_exc = np.zeros((N + 1, startind + len(t)))
    rates_inh = np.zeros((N + 1, startind + len(t)))
...
def timeIntegration_njit_elementwise(
    ...
    for i in range(startind, startind + len(t)):
        ...
        if not distr_delay:
            ...
            for no in range(N):
                ...
                for l in range(N):
                    ...
                    rd_exc[l, no] = rates_exc[no, i - Dmat_ndt[l, no] - 1]  # convert Hz to kHz
                # Warning: this is a vector and not a matrix as rd_exc
                rd_inh[no] = rates_inh[no, i - ndt_di - 1]  # convert Hz to kHz
        ...
        # synaptic currents
        I_et = _syn_exc_current(V_t[i - 1], s_et[0])
        I_gt = _syn_inh_current(V_t[i - 1], s_gt[0])
        I_er = _syn_exc_current(V_r[i - 1], s_er[0])
        I_gr = _syn_inh_current(V_r[i - 1], s_gr[0])
        ...
        I_T_t = g_T_t * m_inf_T_t * m_inf_T_t * h_T_t[0] * (V_t[i - 1] - E_Ca)
        I_T_r = g_T_r * m_inf_T_r * m_inf_T_r * h_T_r[0] * (V_r[i - 1] - E_Ca)

        # h-type current
        I_h = g_h * (m_h1[0] + g_inc * m_h2[0]) * (V_t[i - 1] - E_h)

        ### define derivatives
        # membrane potential
        d_V_t = -(I_leak_t + I_et + I_gt + ext_current_t) / tau_thlm - (1.0 / C_m) * (I_LK_t + I_T_t + I_h)
        d_V_r = -(I_leak_r + I_er + I_gr + ext_current_r) / tau_thlm - (1.0 / C_m) * (I_LK_r + I_T_r)
        # Calcium concentration
        d_Ca = alpha_Ca * I_T_t - (Ca[0] - Ca_0) / tau_Ca
        # channel dynamics
        ...
        d_h_T_t = (h_inf_T_t - h_T_t[0]) / tau_h_T_t
        d_h_T_r = (h_inf_T_r - h_T_r[0]) / tau_h_T_r
        ...
        P_h = k1 * Ca[0] ** n_P / (k1 * Ca[0] ** n_P + k2)
        d_m_h1 = (m_inf_h * (1.0 - m_h2[0]) - m_h1[0]) / tau_m_h - k3 * P_h * m_h1[0] + k4 * m_h2[0]
        d_m_h2 = k3 * P_h * m_h1[0] - k4 * m_h2[0]
        # synaptic dynamics
        d_s_et = ds_et[0]
        d_s_er = ds_er[0]
        d_s_gt = ds_gt[0]
        ...
        rates_exc[-1, i] = _firing_rate(V_t[i])
        rates_inh[-1, i] = _firing_rate(V_r[i])
        Ca[0] = Ca[0] + dt * d_Ca
        h_T_t[0] = h_T_t[0] + dt * d_h_T_t
        h_T_r[0] = h_T_r[0] + dt * d_h_T_r
        m_h1[0] = m_h1[0] + dt * d_m_h1
        m_h2[0] = m_h2[0] + dt * d_m_h2
        s_et[0] = s_et[0] + dt * d_s_et
        s_gt[0] = s_gt[0] + dt * d_s_gt
        s_er[0] = s_er[0] + dt * d_s_er
        s_gr[0] = s_gr[0] + dt * d_s_gr
        # noisy variable
        ds_et[0] = ds_et[0] + dt * d_ds_et + gamma_e ** 2 * d_phi * sqrt_dt * noise_exc[-1]
        ds_gt[0] = ds_gt[0] + dt * d_ds_gt
        ds_er[0] = ds_er[0] + dt * d_ds_er
        ds_gr[0] = ds_gr[0] + dt * d_ds_gr
